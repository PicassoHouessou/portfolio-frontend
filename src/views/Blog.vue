                    ...
                    <div class="col-lg-8 ftco-animate">
                        ...
                        <div class="pt-5 mt-5">
                            ...
                            <div class="comment-form-wrap pt-5">
                                ...
                                <ValidationObserver class="col-md-12" ref="form" v-slot="{ invalid, handleSubmit, reset }">
                                <form action="#" class="p-5 bg-light" @reset.prevent="reset" @submit.prevent="handleSubmit(submit)" id="blogForm" novalidate="novalidate">
                                    <div class="form-group">
                                        <ValidationProvider rules="required|max:200" v-slot="{ errors }">
                                        <label for="fullName" :class="{'validation-error': errors[0] }">Name *</label>
                                        <input type="text" class="form-control" id="fullName" v-model="form.fullName"  :class="{'input-error': errors[0] }">
                                            <span class="validation-error">{{ errors[0] }}</span>
                                        </ValidationProvider>
                                    </div>
                                    <div class="form-group">
                                        <ValidationProvider rules="required|email|max:200" v-slot="{ errors }">
                                        <label for="email"  :class="{'validation-error': errors[0] }">Email *</label>
                                        <input type="email" class="form-control" id="email" v-model="form.email"  :class="{'input-error': errors[0] }">
                                            <span class="validation-error">{{ errors[0] }}</span>
                                        </ValidationProvider>
                                    </div>
                                    <div class="form-group">
                                        <ValidationProvider rules="max:200" v-slot="{ errors }">
                                        <label for="website">Website</label>
                                        <input type="url" class="form-control" :class="{'input-error': errors[0] }" id="website" v-model="form.website">
                                            <span class="validation-error">{{ errors[0] }}</span>
                                        </ValidationProvider>
                                    </div>

                                    <div class="form-group">
                                        <ValidationProvider rules="required|max:2000" v-slot="{ errors }">
                                        <label for="message" :class="{'validation-error': errors[0] }">Message</label>
                                        <textarea name="message" id="message" v-model="form.message" cols="30" rows="10" class="form-control"  :class="{'input-error': errors[0] }"></textarea>
                                            <span class="validation-error">{{ errors[0] }}</span>
                                        </ValidationProvider>
                                    </div>
                                    <div class="form-group">
...
<script>
    ...
    extend('max', {
        ...max,
        message: 'The maximum  {_field_} length is {length} '
    });
    */

const POST_CONTENT_URL = "https://localhost:8000/api/posts" ;
    export default {
        name: 'Blog',
        components: {NavBar, Footer, ValidationObserver, ValidationProvider},
        data(){
            return {
                form: {
                    message: '',
        ...
        methods: {
            ...
            getPostContent() {
                axios.get(POST_CONTENT_URL, {responseType :"json"}).then(response => {
